...
public class CategoryService {
    ...
    public List<ProductDto> getProducts(int cid, int page, int itemPerPage) {
        ...
        for (Object[] row : list) {
            productDtos.add(new ProductDto((Integer) row[0], Double.toString((Double) row[1]), (Double) row[5]));
        }
        session.close();
        return productDtos;
    }
    ...
    public List<CategoryDto> getCategoriesTree(int startingId) {
        ...
        getCategory(0, categoryDtos, session);
    ...
    public void getCategory(int parentId, List<CategoryDto> categoryDtos, Session session) {
        ...
        for (CategoryDto categoryDto : categoryDtos) {
            ...
            getCategory(categoryDto.getId(), categoryDto.getSubcategories(), session);
        }
    }
}
